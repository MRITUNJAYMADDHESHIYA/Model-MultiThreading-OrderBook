...
//Question:- Takes a string, a character count, and a thread count(t1,t2,t3.....) as input via command-line.
#include <iostream>
#include <string>
#include <mutex>
#include <vector>
...
public:
    ...
    void run() {
        for (int i = 0; i < thread_count; i++) {
            thread t(&MyPrinter::print_thread, this);  //different threads
            cout << "Thread " << t.get_id() <<  " is " << i << endl;
            thread_ids.push_back(t.get_id()); //you can see in other function use
            threads.push_back(move(t));  //move because do not copy
        }
