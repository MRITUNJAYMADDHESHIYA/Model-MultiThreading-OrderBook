...
//server gets the message, decode the header, get the bodyLength form the header and hence complete body 
//then server sends the message to all the clients connected to that room.


// Concept	Value	Purpose
// header	4 bytes	Fixed-size. Stores body length as string.
// maxBytes	512 bytes	Maximum message body length.
// data	4 + 512	Stores header + body.
...
class Message{
    public:
        ...
        void printMessage(){ //return full data: header + body
